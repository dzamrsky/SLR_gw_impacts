# -*- coding: utf-8 -*-
"""
Created on Fri Jan 28 11:26:44 2022
...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  5 12:26:01 2020
...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 11 15:01:08 2020
...
model_sc_csv_dir = r'/projects/0/qt16165/_dzamrsky/_A4_SRM_models/_SLR_models/_model_scenarios_GEO.csv'
input_dir = r'/projects/0/qt16165/_dzamrsky/_A4_MODEL_input_files'
cs_regs_shp_dir = r'/projects/0/qt16165/_dzamrsky/_A4_input_data/cs_reg_id_v2.shp'
model_out_dir = r'/projects/0/qt16165/_dzamrsky/_A4_SRM_models/_SLR_models'
...
cs_regs_shp_dir_1 = r'/projects/0/qt16165/_dzamrsky/_A4_input_data/_coastal_points\cs_reg_id.shp'
...
out_dir = r'/projects/0/qt16165/_dzamrsky/_A4_SRM_models/_SLR_models_OUT_files_validation'
input_dir = r'/projects/0/qt16165/_dzamrsky/_A4_MODEL_input_files'
master_csv_dir = r'/projects/0/qt16165/_dzamrsky/_A4_SRM_models/_SLR_models/_SRM_models_mastertable.csv' 
#id_loop = int(sys.argv[1])


#main_dir = r'g:\Water_Nexus\_A4_models\_SLR_models'
#out_dir = r'd:\Water_Nexus\_A4_models\_SLR_models_OUT_files'
#input_dir = r'd:\Water_Nexus\_A4_models\_SLR_models_OUT_files\_input_files'
...
for id_loop in range(5000, 5034):
    ...
    try:
        ...
        for param_combo in [1, 9, 17]:
            ...
            try:
                ...
                cs_regs_shp_dir = os.path.join(cs_regs_shp_dir_1)
                cs_regs_raw = gpd.read_file(cs_regs_shp_dir)
                cs_type = cs_regs_raw.loc[cs_regs_raw['cst_reg_id'] == subreg_id]
                cs_type = cs_type.loc[cs_type['coscat_id'] == coscat_id]['cst_type'].values.tolist()
                cs_type = max(set(cs_type), key=cs_type.count)
        ...
        for i in range(len(rcp_names)):
            ...
            try:
               ...
               topo_dict = xr.open_mfdataset(os.path.join(input_dir, cs_srm_str.split('_')[0], cs_srm_str.split('_')[0] + '_REG_' + cs_srm_str.split('SRM_')[1], '_topo_100m_avg.nc'), combine='by_coords')
            except (KeyError, OSError):
                print('Something is fucked ' + str(id_loop))
                print(os.path.join(input_dir, cs_srm_str.split('_')[0], cs_srm_str.split('_')[0] + '_REG_' + cs_srm_str.split('SRM_')[1], '_gebco_100_IBOUND_IBOUND.npy'))
                continue
        
            try:
                nc_in_coastal = xr.open_mfdataset(os.path.join(avg_nc_folder, cs_srm_str + '_' + rcp_name + '_coastal_MERGED.nc'), combine='by_coords')
                ...
                print(os.path.join(avg_nc_folder, cs_srm_str + '_' + rcp_name + '_coastal_MERGED.nc'))
                """
                try:
            ...
            try:
                nc_in_merit = xr.open_mfdataset(os.path.join(avg_nc_folder, cs_srm_str + '_' + rcp_name + '_merit_MERGED.nc'), combine='by_coords')
                ...
                try:
                    ...
                    y_bot_lst = conc_merit_2000[:, 0].tolist()
                    bot = conc_merit_2000[:, int(abs(round(x_st * 10, 1)))].tolist()   
            ...
            try:
                nc_in_gebco = xr.open_mfdataset(os.path.join(avg_nc_folder, cs_srm_str + '_' + rcp_name + '_gebco_MERGED.nc'), combine='by_coords')
            ...
            try:
                ...
                if conc_coastal_2000 is not None and np.count_nonzero(conc_coastal_2000) != 0:
                    ...
                    try:
                        ...
                        print('NO INLAND AREA EXTENT FOUND FOR ', cs_srm_str)
                        print(x_st, x_end, x_lst)
                ...
                if conc_merit_2200 is not None and np.count_nonzero(conc_merit_2200) != 0:  
                    ...
                    try:
                        ...
                        print('NO INLAND AREA EXTENT FOUND FOR ', cs_srm_str)
                        print(x_st, x_end, x_lst)
                    sww_m_2200 = sww_inland(conc_merit_2200, cst_idx)
                ...
                #   plot the colorbar
                try:
                    cbar = plt.colorbar(im6_1, cax = ax10_1, cmap = cmap, pad=-0.2,  shrink=0.9, aspect=30, norm = norm, spacing = 'uniform', ticks = bounds, boundaries = bounds)
                except NameError:
                    try:
                        ...
                        try:
                            cbar = plt.colorbar(im6_3, cax = ax10_1, cmap = cmap, pad=-0.2,  shrink=0.9, aspect=30, norm = norm, spacing = 'uniform', ticks = bounds, boundaries = bounds)
                        except NameError:
                            continue
    ...
    except(FileNotFoundError, AttributeError, ValueError, UnboundLocalError, IndexError):
        print('ERROR FILE NOT FOUND    ', id_loop)
        pass
