...
input_files_dir = r'projects/0/qt16165/_dzamrsky/_A4_input_data/_A4_input_data'
plot_dir = r'projects/0/qt16165/_dzamrsky/_A4_input_data/_A4_MODEL_input_files/_plots'
model_dir = r'projects/0/qt16165/_dzamrsky/_A4_input_data/_A4_MODEL_input_files'
"""
...
def save_model_input_files_cst_type_topo_v2(cst_type, thk_lst, width_lst, anchor_dist_lst, anchor_depth_lst, wtd_lst, topo_lst, topo_gebco_500m_lst,\
    ...
    if cst_offset_val is None:
        ...
        print("Pica") 
        cst_idx = final_topo.index(cst_offset_val) - 1
        cst_offset = (cst_idx - int((len(avg_topo) - 1) * (500. / topo_res) / 10.)) * topo_res / 1000.
        model_run = True   
        ...
        if cst_offset_val is None:
            ...
            if cst_off_idx < cst_look_up_idx:
                ...
                cst_look_up_idx = int((len(avg_topo) - 1) * (500. / topo_res) / 10.  / 2) + int(((len(avg_topo) - 1) * (500. / topo_res) / 10.  / 2) / 2)
                #   find the position of the coastline
                """implement the new approach, use the sea level in the stress period 10ka BP as limit"""
                #   first check where is the bathymetry location which is lower than that sea level
            
                try:      
                    cst_offset_val_deep  = next((x for x in final_topo[cst_look_up_idx:] if x < -32.), None)
                    cst_offset_idx_deep = final_topo[cst_look_up_idx:].index(cst_offset_val_deep) - 1
                    #   second, go from there in the opposite direction (landward) and look for first cell above current sea level (0)
                    cst_offset_val  = next((x for x in list(reversed(final_topo[:cst_offset_idx_deep + cst_look_up_idx])) if x >= sea_level), None)
                    cst_offset_idx = final_topo[cst_look_up_idx:].index(cst_offset_val) 
                    #cst_offset = ((cst_look_up_idx + cst_offset_idx) - 400) * 0.5 
                    cst_offset = ((cst_look_up_idx + cst_offset_idx) - int((len(avg_topo) - 1) * (500. / topo_res) / 10.)) * topo_res / 1000.
                    cst_off_idx = cst_look_up_idx + cst_offset_idx                
                except ValueError:
            ...
            if model_run == True:
                ...
                try: 
                    ...
                    model = cs_model(1, final_topo, avg_nasa_refined, x_nasa, y_nasa, cst_thick_est_avg, end_cst_plain,\
                                     ...
                                     final_topo, final_topo, final_topo, avg_k_soil_refined,\
                                     avg_drn_rate_refined, avg_glhymps_top_lay_refined, avg_glhymps_bot_lay_refined, topo_res, False)
                    model.get_top_bot_lst_find_cst(cst_look_up_idx, fos[1], 0.0, wtd_cst)
